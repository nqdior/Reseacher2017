...
namespace newtype.Database
{
    ...
    public class MySqlConnecter
    {
        ...
        /// DB情報構造体
        /// </summary>
        public MyDBProvider DBinfo { get; set; }

        /// <summary>
        /// DB(SQLSV)の情報構造体：trn 
        /// </summary>
        private MySqlTransaction trn = null;
        ...
        /// コンストラクタ
        /// </summary>
        public MySqlConnecter(MyDBProvider DBinfo)
        {
            this.DBinfo = DBinfo;
        }
        #endregion
        ...
        public bool ConnectToSQLServer(MyConnection connection, bool isMsgShow = false, 
                                                        bool isErrSkip = false, bool isRetryFlag = false, int maxRetryTimes = 0)
        {
            for (int retryTimes = 0; retryTimes <= maxRetryTimes; retryTimes++)
                try
                {
                    ...
                    DBinfo.sqlConnection = new MySqlConnection();
                                                                     ...
                                                                     bool isErrSkip = false, bool isRetryFlag = false, int maxRetryTimes = 0)
        {
            ...
            for (int retryTimes = 0; retryTimes <= maxRetryTimes; retryTimes++)
                try
                {
                    builder = new MySqlCommandBuilder(DBinfo.adapter);
        ...
        public void DisconnectFromSQLServer(bool isMsgShow = false)
        {
            try
            {
                if (DBinfo.sqlConnection != null)
                {
                    ...
                    MySqlConnection.ClearPool(DBinfo.sqlConnection);
