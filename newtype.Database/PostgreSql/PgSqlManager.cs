...
namespace newtype.Database
{
    ...
    /// </summary>
    public class PgSqlManager : ISqlManager
    {
        private PgSqlConnecter sqlConnecter;

        public PgConnection connection;

        public object Server { get { return connection.Server; } set { connection.Server = (NpgsqlConnectionStringBuilder)value; } }
        ...
        {
            if (ExecuteSqlToServer(Properties.Resource.PGDATABASE, "DataBase"))
            {
        ...
        {
            if (ExecuteSqlToServer(Properties.Resource.PGTABLES, "Tables"))
            {
                Tables["Tables"].Columns[0].ColumnName = "Name";
                Tables["Tables"].Columns.Add("Rows");
                Tables["Tables"].Columns.Add("Columns");
                return true;
            }
            return false;
        }
        ...
        {
            var sql = string.Format(Properties.Resource.PGCOLUMNS, Catalog);
            return ExecuteSqlToServer(sql, "Columns");
        }
